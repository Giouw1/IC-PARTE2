...
def draw_graph(G, color_assignments, file_name=None, save_path=None):
    """Desenha o grafo com as cores atribuídas aos vértices e salva no caminho especificado."""
    ...
    # Se save_path não for especificado, salva na pasta padrão (Downloads)
...
# Função para processar uma instância específica e desenhar o grafo
def process_and_draw_graph(file_path):
    ...
    print(arestas)
    print("\nValor de k:")
    print(k)    # Cria o grafo a partir da lista de adjacências
    G = nx.Graph()
    G.add_nodes_from(vertices)
    G.add_edges_from(arestas)

    # Atribui cores aos vértices, usando os vértices pré-coloridos
    color_assignments = {v: precolored.get(v, -1) for v in vertices}  # -1 para vértices não pré-coloridos

    # Extrair nome do arquivo para nome da imagem
    ...
    draw_graph(G, color_assignments, file_name=file_name, save_path=save_path)

def main():
     file_path = "graph_d_regular_k2_p0.1_v50_20241127_082538.txt"
     process_and_draw_graph(file_path)
